...
public class PAppletGraphDisplayer extends PApplet {
	...
	private String required_algorithm = "QuickSort";
	...
	public void draw() {
		...
		for (int x_axis = 0; x_axis < arrayLength; x_axis++) {
			...
			if (displayer.left != -1) {
				fill(100);
				line(55 + 56 * displayer.left, 270, 55 + 56 * displayer.left,
						330);
			}
			if (displayer.right != -1) {
				fill(100);
				line(55 + 56 * displayer.right, 270, 55 + 56 * displayer.right,
						330);
			}
			// delay(100);
		}
		...
		if (Visualize.start) {
			...
			case "MergeSort":
				save("..\\Image Question Folder\\MergeSort_imagequestion_"
				...
				break;
			case "QuickSort":
				save("..\\Image Question Folder\\QuickSort_imagequestion_"
						+ Serial_number.serialno() + ".png");
				Thread quick = new Thread() {
					public void run() {
						QuickSort quickData = new QuickSort(displayer);
						quickData.start();
						quickData.sort(0, arrayLength - 1);
						quickData.finish();
					}
				};
				quick.start();
				break;
			case "HeapSort":
				save("..\\Image Question Folder\\HeapSort_imagequestion_"
						+ Serial_number.serialno() + ".png");
		...
		if (Visualize.terminate) {
			switch (required_algorithm) {
			case "MergeSort":
				save("..\\Image Solution Folder\\MergeSort_imagesolution_"
						+ Serial_number.serialno() + ".png");
				...
				break;
			case "QuickSort":
				save("..\\Image Solution Folder\\QuickSort_imagesolution_"
						+ Serial_number.serialno() + ".png");
				Visualize.terminate = false;
				...
				break;
			case "HeapSort":
				save("..\\Image Solution Folder\\HeapSort_imagesolution_"
						+ Serial_number.serialno() + ".png");
				Visualize.terminate = false;
